    ...
    <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:py-10">
      <section className="rounded-3xl border border-slate-200 bg-white/90 p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900/90 sm:p-8">
        ...
        <p className="mt-3 max-w-2xl text-sm text-slate-600 dark:text-slate-300 sm:text-base">
          Train by topic, learn formulas, and see every step. Questions are grouped
          by method so you can recognize patterns fast under exam pressure.
        </p>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          >
            Open 3-day crash plan
          </Link>
        </div>

        <label className="mt-6 block text-sm font-medium text-slate-700 dark:text-slate-200">
          Search by keyword, formula, or topic
          <input
            value={search}
            onChange={(event) => setSearch(event.target.value)}
            placeholder="Try: Bayes, z-test, CLT, Poisson"
            ...
            <ul className="mt-2 space-y-2">
              {searchResults.length === 0 ? (
                <li className="text-sm text-slate-600 dark:text-slate-300">
                  No match found. Try a formula keyword like &quot;Phi&quot; or
                  &quot;H0&quot;.
                </li>
              ) : (
        ...
        <div className="space-y-4">
          ...
          <section className="rounded-2xl border border-blue-200 bg-blue-50 p-5 dark:border-blue-900 dark:bg-blue-900/30">
            ...
            <p className="mt-2 text-sm text-blue-900 dark:text-blue-100">
              Prioritized for fast scoring: Standardization, Poisson process,
              Bayes/Total Probability, Combinatorics, Confidence intervals, and
              Hypothesis tests first. Then CLT and MLE.
            </p>
            <Link
              href="/crash-plan"
